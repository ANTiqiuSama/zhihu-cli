                    ...
                    if not _is_captcha_endpoint(path):
                        ...
                        if mode == "auto":
                            ...
                            if result == "resolved":
                                ...
                                resp = super().request(method, url, **kwargs)
                                if detect_captcha(resp) is not None:
                                    raise click.ClickException(
                                        "Zhihu still reports risk-control error 40352 after verification. "
                                        "Import a verified browser session with 'zhihu-cli auth paste'."
                                    )
                            else:
                                raise click.ClickException(
                                    "Zhihu blocked this request with risk-control error 40352. "
                                    "Complete browser verification or run 'zhihu-cli auth paste'."
                                )
                        elif mode == "prompt":
